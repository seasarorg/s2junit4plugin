 ...
 * @since 3.1
 */
public class NewS2JUnit4TestCaseWizardPageOne extends NewS2JUnit4TypeWizardPage {
	 ...
	 * @since 3.1
	 */
	public NewS2JUnit4TestCaseWizardPageOne(NewTestCaseWizardPageTwo page2) {
	...
	private void createConstructor(IType type, ImportsManager imports) throws CoreException {
		...
		GenStubSettings settings= S2JUnit4StubUtility.getCodeGenerationSettings(type.getJavaProject());
		settings.createComments= isAddComments();
		
		if (methodTemplate != null) {
			settings.callSuper= true;				
			settings.methodOverwrites= true;
			content= S2JUnit4StubUtility.genStub(type.getCompilationUnit(), getTypeName(), methodTemplate, settings, null, imports);
		} else {
	...
	private void createSetupStubs(IType type, String methodName, boolean isStatic, String annotationType, ImportsManager imports) throws CoreException {
		...
		if (methodTemplate != null) {
			settings.callSuper= true;
			settings.methodOverwrites= true;
			content= S2JUnit4StubUtility.genStub(type.getCompilationUnit(), getTypeName(), methodTemplate, settings, annotation, imports);
		} else {
			final String delimiter= getLineDelimiter();
			StringBuffer buffer= new StringBuffer();
	...
	private void appendTestMethodBody(StringBuffer buffer, ICompilationUnit targetCu) throws CoreException {
		...
		if (fPage2.isCreateTasks()) {
			String todoTaskTag= S2JUnit4StubUtility.getTodoTaskTag(targetCu.getJavaProject());
			if (todoTaskTag != null) {
	...
	private void appendMethodComment(StringBuffer buffer, IMethod method) throws JavaModelException {
		...
		buf.append('(');
		String[] paramTypes= S2JUnit4StubUtility.getParameterTypeNamesForSeeTag(method);
